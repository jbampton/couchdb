...
job_summary(JobId, HealthThreshold) ->
    case job_by_id(JobId) of
        {ok, #job{pid = Pid, history = History, rep = Rep}} ->
            ...
            Priority = couch_replicator_share:get_priority(JobId),
            [
...
start_priority_key(#job{} = Job) ->
    {couch_replicator_share:get_priority(Job#job.id), last_started(Job)}.
...
job_ejson(Job) ->
    ...
    Priority = couch_replicator_share:get_priority(Job#job.id),
    {[
